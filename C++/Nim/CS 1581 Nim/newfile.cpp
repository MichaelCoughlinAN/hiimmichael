...
gameState computerMove(gameState state) {
    if (sizeOfPile(1) > 0)
        cout << "I take 1 coin from Pile 1.";
        return removeCoinsFromPile(1, 1);
    else
        cout << "I take 1 coin from Pile 2.";
        return removeCoinsFromPile(1, 2);
}
...
gameState humanMove(gameState state) {
    cout << "Which pile will you choose from?";
    int p = prompt(int input); 
    cout << "How many coins will you remove?";
    int n = prompt(input);
    removeCoinsFromPile(n, p);    
}
...
void playWithTurns(gameState state, player p) {
    if over(state) 
    return annouceWinner(p);
    else if (p == human)
        return playWithTurns(humanMovie(state), computer)
    else if
        return playWithTurns(computerMove(state), human)
    else
        cout << "player wasn't human or computer.";
