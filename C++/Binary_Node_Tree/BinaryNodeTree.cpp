...
void BinaryNodeTree<ItemType>::pathSum(int sum) {
  int pathArray[getHeight()+1];
  doesSomePathHaveSum(rootPtr, pathArray, 0, sum);
}

template <typename ItemType>
void BinaryNodeTree<ItemType>::doesSomePathHaveSum(BinaryNode<ItemType>* nodePtr, int pathArray[], int count, int sum) {
    if (nodePtr == nullptr) 
      return; //break from void function
  
    pathArray[count] = getItem(nodePtr);
    count++;

 
    if (nodePtr->leftChildPtr == nullptr && nodePtr->rightChildPtr == nullptr) {
      if(addArray(pathArray, count, sum))
	cout << " True, does sum up to " << sum << endl;
      else cout << " False, does not sum up to " << sum << endl;

      // return addArray(pathArray, count, sum);

      }
    else {
	doesSomePathHaveSum(nodePtr->leftChildPtr, pathArray, count, sum);
	doesSomePathHaveSum(nodePtr->rightChildPtr, pathArray, count, sum);       
}
...
template <typename ItemType>
bool BinaryNodeTree<ItemType>::addArray(int pathArray[], int count, int sum) {
 int temp = 0;
...
template <typename ItemType>
void BinaryNodeTree<ItemType>::printPaths() {
  ItemType pathArray[getHeight() +1];
  printRootToLeafPathsHelper(rootPtr, pathArray, 0);
}

template <typename ItemType>
void BinaryNodeTree<ItemType>::printRootToLeafPathsHelper(BinaryNode<ItemType>* nodePtr, ItemType pathArray[], int count) { 
    if (nodePtr == nullptr) 
      return; //break from void function
    //add node to array
    pathArray[count] = getItem(nodePtr);
    ...
    //it's a leaf
    if (nodePtr->leftChildPtr == nullptr && nodePtr->rightChildPtr == nullptr) {
	printArray(pathArray, count);
      }
    else
      {
	//both
	printRootToLeafPathsHelper(nodePtr->leftChildPtr, pathArray, count);
	printRootToLeafPathsHelper(nodePtr->rightChildPtr, pathArray, count);
      }
  
}
		
template <typename ItemType>
void BinaryNodeTree<ItemType>::printArray(ItemType pathArray[], int count) {
    for (int i = 0; i < count; i++) {
      cout << pathArray[i] << " "; 
    }
    cout << endl;
...
template <typename ItemType>
ItemType BinaryNodeTree<ItemType>::maxHelper(BinaryNode<ItemType>* nodePtr)
  throw(PrecondViolatedExcep) {
 
  if(nodePtr == nullptr) 
    throw PrecondViolatedExcep();
  else if(nodePtr->rightChildPtr == nullptr)
    return getItem(nodePtr);
  else return maxHelper(nodePtr->rightChildPtr);
 }
...
template <typename ItemType>
ItemType BinaryNodeTree<ItemType>::minHelper(BinaryNode<ItemType>* nodePtr)
  throw(PrecondViolatedExcep) {
 
  if(nodePtr == nullptr)
    throw PrecondViolatedExcep(); //empty tree
  else if(nodePtr->leftChildPtr == nullptr)
    return getItem(nodePtr); //no left child
  else return minHelper(nodePtr->leftChildPtr);//has left child
 }
...
ItemType BinaryNodeTree<ItemType>::minValue(){
  return minHelper(rootPtr);
}

template <typename ItemType>
void BinaryNodeTree<ItemType>::flipTree(BinaryNode<ItemType>* nodePtr) {
  BinaryNode<ItemType>* tempPtr = new BinaryNode<ItemType>;

 if (nodePtr != nullptr) {
    tempPtr = nodePtr->leftChildPtr;
    nodePtr->leftChildPtr = nodePtr->rightChildPtr;
    nodePtr->rightChildPtr = tempPtr;    
    flipTree(nodePtr->leftChildPtr);
    flipTree(nodePtr->rightChildPtr);
  }
}
...
template <typename ItemType>
bool BinaryNodeTree<ItemType>::isBinTreeHelper(BinaryNode<ItemType>* nodePtr, int min, int max) {

   if (nodePtr == nullptr) 
     return true;
   if(getItem(nodePtr) < min || getItem(nodePtr) > max)
     return false; 
   else {
 return   
   isBinTreeHelper(nodePtr->leftChildPtr, min, getItem(nodePtr)-1) &&
   isBinTreeHelper(nodePtr->rightChildPtr, getItem(nodePtr)+1, max);
   }
}

template <typename ItemType>
bool BinaryNodeTree<ItemType>::isBinTree() {
  return isBinTreeHelper(rootPtr, INT_MIN, INT_MAX);
}
